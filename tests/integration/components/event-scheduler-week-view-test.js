...
module('Integration | Component | event-scheduler | Week view', function(hooks) {
  ...
  module('event drop test', function(hooks) {
    ...
    hooks.beforeEach(async function() {
      ...
      viewStart = this.currentDate.clone().startOf('week');
    ...
    test('should show popup when prev start time is empty and ignore the drop action when user failed to set the start time', async function(assert) {
      ...
      return wait().then(async() => {
        ...
        assert.equal(find(`[data-resource-id="${this.resourceId}"]`).querySelectorAll(SELECTORS.event).length, 0);
      });
    });
    ...
    test('should place the event at the last cell when dropped between 0 to 100% of the last cell', async function(assert) {
      assert.dom(element).hasClass('activated');
      let eventDropPosition = (7 * slotWidth) - 5;
      startTime = viewStart.clone().add({ days: 6, hours: 4 }).toISOString();
      endTime = viewStart.clone().add({ days: 6, hours: 7 }).toISOString();
